// Tell the user that this cite is outdated
alert('This site is oudated. Check out www.jeffreychan.dev for the updated site');
...
window.addEventListener('scroll', () => {
  ...
  target.style.transform =
    `translate3d( 0vw, ${-20 * rate}vh, ` +
    10 * rate +
    `vw) scale3d( ${1 - rate / 20}, ${1 - rate / 20}, 1)`;

  if (rate >= 0.25) {
    target.style.opacity = `${1 - 2.2 * (rate - 0.25)}`;
    target.style.filter = `blur(${10 * (rate - 0.35)}px)`;
...
function infoAnimation(element, topPosition, bottomPosition) {
  if (topPosition < 0 && topPosition > -0.25) {
    element.style.opacity = `${-5 * topPosition - 0.2}`;
    element.style.filter = `blur(${3 - -topPosition * 12}px)`;
  } else if (bottomPosition > 0 && bottomPosition < 0.25) {
    element.style.opacity = `${5 * bottomPosition - 0.2}`;
